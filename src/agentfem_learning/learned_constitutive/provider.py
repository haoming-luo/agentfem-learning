...
class TorchConstitutiveProvider:
    ...
    _cache: dict[tuple[str, str, str], object] = field(default_factory=dict, init=False)
    _load_seconds: dict[tuple[str, str, str], float] = field(
        default_factory=dict, init=False
    )

    def _validate_specification(self, specification, bundle: ModelBundle) -> None:
        ...
        if tuple(manifest["voigt_order"]) != tuple(specification.tangent_convention.component_order):
        ...
        expected_parameters = {
            str(item["name"]): item for item in manifest["parameter_schema"]
        }
        declared_parameters = {
            item.name: item for item in specification.parameter_schema
        }
        ...
        available = {
            name for name in ("stress", "state", "batch", "energy", "diagnostics")
        ...
        if dict(manifest["applicability_domain"]) != dict(
            specification.applicability_domain
        ):
    ...
    def create(self, specification):
        ...
        if material.tangent_convention != specification.tangent_convention:
            raise ModelBundleError("Tangent convention differs between specification and bundle.")
        return material

    def evidence(self, specification) -> dict[str, object]:
        bundle = load_model_bundle(Path(specification.artifact))
        key = (bundle.manifest_sha256, self.runtime.device, self.runtime.dtype)
        return {
            **bundle.summary(),
